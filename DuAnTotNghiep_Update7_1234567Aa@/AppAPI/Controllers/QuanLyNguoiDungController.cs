...
namespace AppAPI.Controllers
{
    ...
    public class QuanLyNguoiDungController : ControllerBase
    {
        ...
        public QuanLyNguoiDungController(IMailServices mailServices)
        {
        ...
        public async Task<IActionResult> DangKyKhachHang(KhachHangViewModel khachHang)
        {
            ...
            try
            {
                _mailService.SendMail(new AppData.ViewModels.Mail.MailData()
                {
                    EmailSubject = "Shop Thời Trang - Kích Hoạt Tài Khoản ",
                    EmailToId = khachHang.Email,
                    EmailToName = khachHang.Name,
                    EmailBody = "Đăng ký tài khoản thành công ! Vui lòng nhấn vào link <a href=\"https://localhost:5001/Home/KichHoat?Id=" + kh.IDKhachHang + "\"></a> để kích hoạt tài khoản !"
                });
            }
            catch (Exception ex) { 
        ...
        [HttpGet("KichHoat")]
        public async Task<IActionResult> KichHoat(Guid Id)
        {
            var kh = await service.KichHoat(Id);
