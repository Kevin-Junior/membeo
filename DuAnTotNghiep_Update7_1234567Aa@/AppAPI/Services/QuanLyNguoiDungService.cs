    ...
    public class QuanLyNguoiDungService : IQuanLyNguoiDungService
    {
        ...
        private async Task<bool> SendEmail(string email, string subject, string body)
        {
            try
            {
                ...
                smtpClient.Credentials = new NetworkCredential("hoahuongduong05124@gmail.com", "ytotxwzbrwkoddjd");
                var messsage = new MailMessage();
                messsage.From = new MailAddress("hoahuongduong05124@gmail.com");
        ...
        public async Task<LoginViewModel> Login(string lg, string password)
        {
            try
            {
                var nv = await context.NhanViens
                    .FirstOrDefaultAsync(a => (a.Email == lg || a.SDT == lg) /*&& a.PassWord == password*/ );
                ...
                var kh = await context.KhachHangs
                    .Where(x => x.DaKichHoat == true)
                    .FirstOrDefaultAsync(x => (x.Email == lg || x.SDT == lg) /*&& x.Password == password*/);
        ...
        public async Task<KhachHang> KichHoat(Guid id)
        {
            KhachHang obj = context.KhachHangs
                .Where(x => x.DaKichHoat == false)
