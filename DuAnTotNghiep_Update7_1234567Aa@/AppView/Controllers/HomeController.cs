...
namespace AppView.Controllers
{
    public class HomeController : Controller
    {
                ...
                if (String.IsNullOrEmpty(session))
                {
                    ...
                    if (Request.Cookies["Cart"] != null)
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            TempData["GiamGia"] = temp.GiamGia.ToString();
                            // lam end
        ...
        [HttpGet]
        public IActionResult KichHoat(Guid Id)
        {
            try
            {
                HttpResponseMessage response = _httpClient.GetAsync(_httpClient.BaseAddress + $"QuanLyNguoiDung/KichHoat?Id=" + Id).Result;
                if (response.IsSuccessStatusCode)
                {
                    TempData["SuccessMessage"] = "Tài khoản đã được kích hoạt thành công!";
                    //return RedirectToAction("Login", new { actionName = "Index" });
                    return View("ActiveSuccess");
                }
                ViewBag.ErrorMessage = "Thông tin khích hoạt không tồn tại hoặc tài khoản đã được kích hoạt!";
                return View("ActiveSuccess");
            }
            catch
            {
                ViewBag.ErrorMessage = "Kích Hoạt không thành công !";
                return View("ActiveSuccess");
            }
        }
