...
namespace AppView.Controllers
{
    public class NhanVienController : Controller
    {
        ...
        {
            var dsVaiTro = dBContext
                .VaiTros.Select(x => new SelectListItem()
                {
                    Text = x.Ten,
                    Value = x.ID.ToString()
                });

            ViewBag.DSVaiTro = new SelectList(dsVaiTro, "Value", "Text");
        ...
        public async Task<IActionResult> Create(NhanVien nhanVien)
        {
            try
            {
                ...
                string apiUrl = $"https://localhost:7095/api/NhanVien/DangKyNhanVien?ten={nhanVien.Ten}&email={nhanVien.Email}&password={nhanVien.PassWord}&sdt={nhanVien.SDT}&diachi={nhanVien.DiaChi}&IDVaiTro={nhanVien.IDVaiTro}";
