...
namespace AppView.Controllers
{
    public class PhieuNhapController : Controller
    {
        ...
        public async Task<IActionResult> Show(int ProductPage = 1)
        {
            try
            {
                string apiUrl = $"https://localhost:7095/api/PhieuNhap/GetAll";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
                var dsPhieuNhap = JsonConvert.DeserializeObject<List<PhieuNhap>>(apiData);
                return View(new PhanTrangBase<PhieuNhap>
                {
                    Items = dsPhieuNhap
        ...
        public async Task<IActionResult> SearchTheoMa(string? Ma, int ProductPage = 1)
        {
            try
            {
                ...
                string apiUrl = $"https://localhost:7095/api/PhieuNhap/TimKiemPhieuNhap?Ma={Ma}";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
                var users = JsonConvert.DeserializeObject<List<PhieuNhap>>(apiData);
                if (users.Count == 0)
                {
                    ViewData["SearchError"] = "Không tìm thấy kết quả phù hợp";
                }
                return View("Show", new PhanTrangBase<PhieuNhap>
                {
                    Items = users
                             .Skip((ProductPage - 1) * PageSize).Take(PageSize),
                    PagingInfo = new PagingInfo
        ...
        public IActionResult Create()
        {
            ...
            model.ChiTietPhieuNhaps = new List<ChiTietPhieuNhap>();
            model.ChiTietPhieuNhaps.Add(new ChiTietPhieuNhap() { 
                ID = Guid.NewGuid()
            });
        ...
        public async Task<IActionResult> Create(PhieuNhap kc)
        {
            var dsNhanVien = _dbContext.NhanViens
                .Select(x => new SelectListItem()
                {
            ...
            try
            {
                ...
                string apiUrl = $"https://localhost:7095/api/PhieuNhap/Create";
        ...
        public IActionResult Edit(Guid id)
        {
            try
            {
               ...
               .Select(x => new SelectListItem()
               {
                ...
                string apiUrl = $"https://localhost:7095/api/PhieuNhap/GetById/{id}";
                var response = _httpClient.GetAsync(apiUrl).Result;
                var apiData = response.Content.ReadAsStringAsync().Result;
                var model = JsonConvert.DeserializeObject<PhieuNhap>(apiData);
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Guid id, PhieuNhap nv)
        {
            var dsNhanVien = _dbContext.NhanViens
                .Select(x => new SelectListItem()
                {
            ...
            try
            {
                ...
                string apiUrl = $"https://localhost:7095/api/PhieuNhap/Update";
        ...
        {
            string apiUrl = $"https://localhost:7095/api/PhieuNhap/Delete/{id}";
